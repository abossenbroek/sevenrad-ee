"""DSPy Greenhouse Detection Optimization Pipeline (40 Examples).
...
This pipeline includes Sections 3-4 for manual data expansion, allowing comparison
with the 19-example baseline to quantify dataset size impact on:
...
Usage:
    uv run marimo edit notebooks/dspy_optimization_40examples.py
"""
...
def __():
    ...
    directories = {
        ...
        "results/40examples": "40-example results",
        "results/40examples/visualizations": "Charts and plots",
    }
    ...
    console.print(
        Panel.fit(
            "[bold cyan]Section 1: Setup (40-Example Pipeline)[/bold cyan]",
            border_style="cyan",
        )
    )
...
def __(console, train_set, val_set, optimizer_config, GEPA, json, Path, mo, Panel, Progress, SpinnerColumn, TextColumn, dspy, dutch_aware_hierarchical_f1):
    ...
    if optimization_success:
        ...
        optimized_program.save("results/40examples/optimized_program.json")
        console.print("\n[green]✅ Optimization complete![/green]")
        console.print("[green]✅ Saved to results/40examples/optimized_program.json[/green]\n")
    else:
...
def __(console, optimized_program, train_set, val_set, json, Path, mo, Panel, Progress, SpinnerColumn, TextColumn, Table, dutch_aware_hierarchical_f1):
    ...
    # Save CV results
    Path("results/40examples/cv_results.json").write_text(json.dumps(cv_results, indent=2))
...
def __(console, optimized_program, cv_results, json, Path, mo, Panel, Table):
    ...
    report = f"""# DSPy Greenhouse Detection Optimization Report (40 Examples)
...
Optimized model saved to: `results/40examples/optimized_program.json`

Load model in production:
```python
optimized_program = GreenhouseDetector()
optimized_program.load("results/40examples/optimized_program.json")
```

🤖 Generated with Claude Code
"""

    Path("results/40examples/optimization_report.md").write_text(report)
    console.print("[green]✅ Report saved to results/40examples/optimization_report.md[/green]\n")
