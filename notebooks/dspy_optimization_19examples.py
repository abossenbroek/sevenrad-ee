"""DSPy Greenhouse Detection Optimization Pipeline (19 Examples).

This Marimo notebook runs the optimization pipeline with the EXISTING 19 companies
to establish a baseline for comparison with larger dataset experiments.

Workflow:
1. Re-classifying existing 19 companies with fixed evidence classifier
2. Dataset preparation (19 examples, expect high variance)
3. Configuring and running GEPA optimizer
4. Executing repeated nested cross-validation (expect overfitting)
5. Generating final validation report

Expected Results:
- Higher overfitting (train-val gap >15%)
- Higher variance in cross-validation (std >5%)
- Baseline for comparing with 40-example pipeline

Usage:
    uv run marimo edit notebooks/dspy_optimization_19examples.py
"""
...
@app.cell
def __():
    """Section 1: Setup (19-Example Pipeline)."""
    import json
    import os
    ...
    console.print(
        Panel.fit(
            "[bold cyan]Section 1: Setup (19-Example Pipeline)[/bold cyan]",
            border_style="cyan",
        )
    )
...
def __(mo, changed_count, false_positives):
    ...
    **Next:** Proceed to Section 3 for HITL validation
    """)
    return
...
def __(console, json, Path, Panel, Table):
    """Section 3: HITL Validation - Evidence Review."""
    console.print("\n")
    console.print(
        Panel.fit(
            "[bold cyan]Section 3: HITL Validation - Evidence Review[/bold cyan]",
            border_style="cyan",
        )
    )

    research_dir3 = Path("data/research")
    all_research_files = list(research_dir3.glob("*.json"))
    console.print(
    ...
    for file_path3 in all_research_files:
        with open(file_path3) as f3:
            data3 = json.load(f3)
        companies_for_review.append(
            {
                "company": data3["company"],
                "auto_category": data3.get("classification_suggestion", "UNKNOWN"),
                "confidence": data3.get("confidence_score", 0.0),
                "file": str(file_path3),
            }
        )
    ...
    console.print(
        "[dim]Update JSON files manually as needed, then proceed to Section 4[/dim]\n"
    )
...
def __(console, json, Path, dspy, Panel, Table):
    """Section 4: Dataset Preparation & Quality Check."""
    from sklearn.model_selection import train_test_split

    console.print("\n")
    console.print(
        Panel.fit(
            "[bold cyan]Section 4: Dataset Preparation & Quality Check[/bold cyan]",
            border_style="cyan",
        )
    )

    # Load all validated companies
    research_dir4 = Path("data/research")
    all_examples = []

    for file_path4 in research_dir4.glob("*.json"):
        with open(file_path4) as f4:
            data4 = json.load(f4)

        # Convert to DSPy Example
        example = dspy.Example(
            location_name=data4["company"],
            location_area=data4["location"],
            is_greenhouse="YES",  # Assume all are greenhouses for now
            uses_growlight=data4[
...
@app.cell
def __(mo, all_examples, train_set, val_set):
    """Section 4 Display."""
    mo.md(f"""
    ## Section 4: Dataset Preparation Complete

    - Total: {len(all_examples)} examples (19 examples baseline)
    - Train: {len(train_set)} | Val: {len(val_set)}

    **Next:** Proceed to Section 5 for GEPA configuration
    """)
    return
...
def __(console, train_set, val_set, json, Path, mo, Panel, Table, dspy):
    """Section 5: GEPA Optimizer Configuration."""
    try:
    ...
    console.print(
        Panel.fit(
            "[bold cyan]Section 5: GEPA Optimizer Configuration[/bold cyan]",
            border_style="cyan",
        )
    )
...
def __(mo):
    """Section 5 Display."""
    mo.md("""
    ## Section 5: GEPA Configuration Complete

    Optimizer configured with Gemini 2.5 Pro teacher model.

    **Next:** Proceed to Section 6 to run optimization (long-running)
    """)
    return
...
def __(console, train_set, val_set, optimizer_config, GEPA, json, Path, mo, Panel, Progress, SpinnerColumn, TextColumn, dspy, dutch_aware_hierarchical_f1):
    """Section 6: Run GEPA Optimization."""
    from sevenrad_ee.ai.dspy_greenhouse import GreenhouseClassification

    console.print("\n")
    console.print(
        Panel.fit(
            "[bold cyan]Section 6: Run GEPA Optimization[/bold cyan]",
            border_style="cyan",
        )
    )
    console.print(
...
def __(mo, optimization_success):
    """Section 6 Display."""
    mo.md(f"""
    ## Section 6: Optimization {'Complete' if optimization_success else 'Failed'}

    {'✅ Optimized model saved' if optimization_success else '❌ Using base model'}

    **Next:** Proceed to Section 7 for cross-validation
    """)
    return
...
def __(console, optimized_program, train_set, val_set, json, Path, mo, Panel, Progress, SpinnerColumn, TextColumn, Table, dutch_aware_hierarchical_f1):
    """Section 7: Repeated Nested Cross-Validation."""
    from sevenrad_ee.ai.cross_validation import repeated_nested_cv

    console.print("\n")
    console.print(
        Panel.fit(
            "[bold cyan]Section 7: Repeated Nested Cross-Validation[/bold cyan]",
            border_style="cyan",
        )
    )
    ...
    # Save CV results
    Path("results/19examples/cv_results.json").write_text(json.dumps(cv_results, indent=2))
...
def __(console, optimized_program, cv_results, json, Path, mo, Panel, Table):
    """Section 8: Final Validation & Report Generation."""

    console.print("\n")
    console.print(
        Panel.fit(
            "[bold cyan]Section 8: Final Validation & Report Generation[/bold cyan]",
            border_style="cyan",
        )
    )
...
"""
    ...
    console.print("[green]✅ Report saved to results/19examples/optimization_report.md[/green]\n")
